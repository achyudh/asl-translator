...
def classify_images(foldername="data/cropped/", num_users=16, num_classes=24):
    ...
    for t0 in range(0, num_groups):
        ...
        print("RFC1")
        rfc_classifier1 = RandomForestClassifier(n_estimators=4000, max_features='sqrt', n_jobs=8, warm_start=True)
        # rfc_classifier1.fit(x_train, y_train)
        # curr_score = rfc_classifier1.score(x_crossval, y_crossval)
        # print(curr_score)

        print("RFC2")
        rfc_classifier2 = RandomForestClassifier(n_estimators=6000, max_features='log2', n_jobs=8, warm_start=True)
        # rfc_classifier2.fit(x_train, y_train)
        # curr_score = rfc_classifier2.score(x_crossval, y_crossval)
        # print(curr_score)

        # print("RFC3")
        # rfc_classifier3 = RandomForestClassifier(n_estimators=500, max_features=10**-4, n_jobs=7, warm_start=True)
        # rfc_classifier3.fit(x_train, y_train)
        # curr_score = rfc_classifier3.score(x_crossval, y_crossval)
        # print(curr_score)

        print("VotingClassifier")
        voting_classifier = VotingClassifier(estimators=[('rfsq', rfc_classifier1), ('rflog', rfc_classifier2)], voting='soft')
