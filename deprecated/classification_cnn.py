...
def classify_images(foldername="data/cropped/", num_users=20, num_classes=5):
    ...
    hog_dataset = unpickle_features("data/cropped_hog_2x2.pkl")
    for t0 in range(0, num_groups):
        ...
        x_train = np.array([x[0] for x in x_train]).reshape((len(x_train), image_dimensions, image_dimensions, 1))
        y_crossval = np_utils.to_categorical([x[1] for x in x_crossval], num_classes)
        x_crossval = np.array([x[0] for x in x_crossval]).reshape((len(x_crossval), image_dimensions, image_dimensions, 1))

        print("Size of Training Set:", len(x_train), "\nSize of Crossval Set:", len(x_crossval))
        print("Training CNN Classifier...")
        test_score = train_cnn1(x_train, y_train, x_crossval, y_crossval, num_classes, input_shape=(image_dimensions, image_dimensions, 1))
